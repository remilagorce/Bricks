...
Embedded from [alirezarezvani/claude-skills](https://github.com/alirezarezvani/claude-skills)
(`marketing/landing`, MIT). Invoked by the `create-landing-page` brick — not
user-invocable on its own.

Generate a polished, self-contained `.html` landing page from a text prompt or
brief. The output is ONE HTML file: all CSS inline in `<style>`, all JS inline in
`<script>`, only external dependencies being Google Fonts + GSAP via CDN. The
page is visually distinctive, animated, and production-quality.

**When `create-landing-page` hands you a pre-filled brief:** skip Phase 0
entirely — all intake answers are final. Go straight to generation.

Plugin paths for scripts and references:

```
${CLAUDE_PLUGIN_ROOT}/skills/create-landing-page/scripts/
${CLAUDE_PLUGIN_ROOT}/skills/create-landing-page/references/
```

## Phase 0: Grill-Me Intake (4 forcing questions, one at a time)

*Skipped when called from `create-landing-page` — the parent brick pre-fills
every slot from workspace context.*

Dependency-ordered. Each question carries explicit "why I'm asking". Stop
condition: max 4.
...
If only primary provided, derive accent algorithmically (lighten 15% for accent; darken 8% for navy-mid; convert to rgba at 0.12 alpha for glow). Use `brand_palette_validator.py` for the deterministic derivation.
...
- **Path:** caller sets `${OUTPUT_DIR}/<product-name-kebab>.html` (Bricks default: `<workspace>/landing-pages/`)
- **Filename:** lowercase kebab-case from product name ("Quill AI" → `quill-ai.html`). Use `kebab_slug_generator.py` for deterministic slug generation + duplicate detection.
...
| Existing file at output path | Append date suffix (kebab_slug_generator.py flags duplicates) |
...
| Script | Role |
|---|---|
| `scripts/brand_palette_validator.py` | Validates HEX format, checks WCAG AA contrast, generates derived palette from primary. |
| `scripts/kebab_slug_generator.py` | Product name → kebab-case filename + duplicate detection in output dir. |
| `scripts/html_validator.py` | Post-generation structural check. |
