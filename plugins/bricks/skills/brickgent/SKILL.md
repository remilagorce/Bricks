...
1. **Look at the table**: `db.py schema <table>` + `db.py select
   <table> --limit 3` (§4) — the real column names come from here.
2. **Generate the prompt** from the user's request — the
   ...
   lines, confirm ONCE.
3. **Dry-run (the iron gate §5)** — rows whose status column is empty or
   `pending` are eligible automatically (no init step needed); the
   preview computes the first 10 rows, streams each result as it
   finishes, writes NOTHING. While the command runs, **read stderr line
   by line** and relay each `preview_row` to the user so they see
   progress — do not wait for the final stdout JSON:
...
4. **Confirmation (GO)** → same command with `--commit` (drop
   `--limit`) — the whole table, rows in parallel, statuses as the
   checkpoint: re-running the same command resumes the pending rows.
   The commit recomputes the 10 pilot rows — accepted cost, announce it
   with the budget (§7: N rows × model × pages worst case; observed
...
subagent's disposable context (§1: the mass never rides the session),
...
§4. Cap: a handful of rows (~5-10) — beyond that, import into the base
and go through the engine (brackets + dry-run + commit).

## Close the run

Receipt: reconciled counts ("512 eligible = 480 done + 22 not_found +
10 failed (top reasons)"), spend vs announced, and the resume line
("re-lancer la même commande reprend les pending"). Max 3 sample rows.
