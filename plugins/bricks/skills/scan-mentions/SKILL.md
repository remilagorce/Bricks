...
if its tools are available. (Single-question, in-session work: the iron
gate does not apply — nothing is written to the database.)
...
The plugin bundles the hosted server (`.mcp.json`) — it needs
`BRIGHTDATA_API_TOKEN` (free token at brightdata.com, 5,000
requests/month, no credit card; store it in `~/.bricks/env`). If the tools
are missing, guide the user, remind them to restart the session, and stop.
If the user cannot set it up now, offer the degraded fallback: built-in
WebFetch, static pages only — their call.
