...
Report state only — never create anything here. Bricks stores GTM data in a
./bricks/ folder in the working directory; creating it eagerly on every
session would litter unrelated directories. Initialization is LAZY: the
workspace skill creates the root + workspace on the first GTM action, in the
right place.
...
def main() -> int:
    ...
    try:
        ...
        if not status.get("initialized"):
            lines.append(
                "- Workspace: Bricks is not initialized in this directory (no "
                "bricks/ yet). Nothing is created until you act. If the user "
                "asks for GTM work, /bricks:workspace auto-initializes it."
            )
        elif not status.get("current"):
            lines.append(
            ...
            name = status["current"]
            goal = f" — goal: {status['goal']}" if status.get("goal") else ""
