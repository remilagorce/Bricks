...
Runner step (per-row, deterministic, zero model):
    python3 runner.py --table companies --step tools/providers/firmo.py:step \
        --status-col firmo_status
    # reads row["name"] (+ optional row["hint"], row["siren"]),
    # returns flat columns: firmo_confidence, siren, legal_name, naf,
    # industry, employees, city, postal, country, executives (JSON),
    # parent_company, firmo_candidates (only when ambiguous)
...
def step(row: dict, ctx: dict) -> dict:
    """Runner step contract: one lookup, flat columns to write on the row."""
    name = (row.get("name") or row.get("company_name") or "").strip()
    if not name:
        raise ValueError("missing input column 'name' for this row")
    hint = (row.get("hint") or " ".join(
        str(row.get(k)) for k in ("postal", "city", "domain") if row.get(k))).strip()
    out = lookup(name, hint, row.get("siren", ""))
    fields = {"firmo_confidence": out.get("confidence")}
    if out.get("error"):
        fields["firmo_error"] = out["error"]
    if out["confidence"] == "ambiguous":
        fields["firmo_candidates"] = json.dumps(out["candidates"], ensure_ascii=False)
        return fields
    for col in ("siren", "legal_name", "company_category", "naf", "industry",
                "employees", "employees_year", "city", "postal", "country",
                "parent_company"):
        if out.get(col) is not None:
            fields[col] = out[col]
    if out.get("executives"):
        fields["executives"] = json.dumps(out["executives"], ensure_ascii=False)
    return fields
