...
| 0.22.4 | run-h : la vague d'identité re-vérifiait LES MÊMES marques de cabinets à chaque workspace de test (Voluntae : 3 runs de suite, ~1 recherche web chacune) — l'auto-apprentissage `exclude_employers` est par-workspace, le savoir s'évapore sur chaque workspace neuf | `jobs.py` **KNOWN_STAFFING_BRANDS** : la mémoire CUMULATIVE du field-test loop au niveau plugin — 25 marques web-vérifiées des runs c/f/h (voluntae, noviac, huca, wiico, adsearch, s&you, nextep, raisehup, team.is, timtargett, jobmania, humanae, morgan philips, hermesiane, tyls…), matching nom-seul word-boundary + token `hr` ajouté ; testé 27/27 cabinets flagués, 20/20 vraies boîtes des runs passent (0 faux positif) · SKILL : le self-healing devient à DEUX niveaux — workspace (kill flags → exclude_employers) + plugin (cabinet web-vérifié → le receipt le signale pour append à KNOWN_STAFFING_BRANDS) |
| 0.22.3 | contrôle du scénario essais-cliniques (283 sponsors biotech, run sur un plugin antérieur) — le run a fini par réussir (231 contacts / 122 boîtes / ~34 crédits) mais en improvisant : (a) l'adaptateur `fullenrich_people` du REPO renvoie 400 sur chaque appel (l'API veut des objets `{value:…}`, on envoie des chaînes brutes) — le run l'a diagnostiqué au curl et **patché dans le cache local seulement** → régression garantie à la prochaine update ; (b) tempête de 429 (8 workers, puis 0,35 s série : 223 puis 163 rows failed) → 2 s a tenu à 0 échec ; (c) `int()` tronquait les crédits fractionnaires (0,25 → 0 : receipts sous-déclarés, §8) ; (d) le mode volume d'enrich-buying-committee (sous-agents session + Bright Data SERP) est MORT à 283 boîtes (connecteur rejeté après ~2-7 appels, contacts trouvés perdus avec les sous-agents tués) — la lane moteur improvisée a couvert toute la base ; (e) mots-clés anglais seuls sur marché FR → boîtes valides silencieusement exclues (la question de Robin) | Portés au repo : **(a)** `call_api` envoie `{value:…}` (survit aux updates ; stub-testé) · **(b)** `RATE_SLEEP` 0,35→2 s + UN retry poli sur 429 (30 s) avant échec propre de la row · **(c)** crédits en float, arrondis à 2 déc. aux 3 receipts · **(d)** enrich-buying-committee : la lane MOTEUR (`--action fetch fullenrich_people`, cap 1-2, preview→GO) devient LE mode volume ≳40 ; les sous-agents session = fallback sans clé uniquement, et ils écrivent en staging AU FIL DE L'EAU (un sous-agent tué n'emporte plus ses trouvailles) · **(e)** doctrine « les vagues parlent la langue du marché » (EN + local dans chaque groupe) dans find-company-people ET enrich-buying-committee — la cascade ne re-requête que les manqués, `person_key` dédup : plusieurs recherches sans jamais payer deux fois la même boîte |
| 0.22.2 | A/B run-f (hiring-test-f) — **verdict scénario 2 : total 1115 s vs ~1940 s (−43 %), et l'étape 2 n'a découvert AUCUN cabinet (287 s)** : le nettoyage se fait enfin en amont. Mais le run tournait encore sur le cache 0.22.0 (Rule 3 — bonus rank plat, vague d'identité absente), et 4 findings : (1) 🔴 le bonus volume a INVERSÉ le tri — les multi-offres (cabinets, gros distributeurs) ont trusté le committed pendant que chaque PME cible mono-offre plafonnait à 45 < cut 49 → toutes parkées, repêchées à la main ; (2) intitulés cabinet (« chef de mission », « collaborateur comptable ») appris à la main ; (3) `check` ne balaie que France Travail → les comptes sourcés HelloWork ressortent « quiet » à tort ; (4) cap 30 requêtes atteint → hunt relancé en double | **(1)** curate : le volume ne GATE plus jamais — commit/park décidé sur un `gate_score` sans volume (reachable 75/60, cut ~39), le volume continue de booster `hiring_score` (signal de priorité pour rank) ; testé : la PME mono-offre à 45 est enfin committée, DEXIS committé au gate 65 / hiring 80 · **(2)** `TITLE_CABINET_RE` (chef de mission, collaborateur comptable) + « notre cabinet », « portefeuille de dossiers », « nos mandants » dans DESC_AGENCY_RE · **(3)** `check` interroge FT **et** HelloWork par nom · **(4)** SKILL : `--max-queries ≥ titres × zones × boards` dimensionné d'entrée |
